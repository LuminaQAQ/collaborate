...
favoriteRouter.get("/favoriteList", jwtMiddleware, async (req, res, next) => {
  const { favorite_group_id, offset, limit } = req.query;
  console.log(favorite_group_id, offset, limit);
  ...
  try {
    ...
    let favoriteList = await db("favorites")
      ...
      .where({
        user_id: req.user.id,
        group_id: Number(favorite_group_id) === 0 ? null : favorite_group_id,
      })
      .offset(offset)
      .limit(limit);
      ...
      favoriteList.map(async (item) => {
        if (item.target_type === "Doc") {
          item.target_info = await db("docs")
            .select(["id", "title", "book_id", "creator_id", "created_at"])
            .where({ id: item.target_id })
            .first();
        } else if (item.target_type === "Book") {
